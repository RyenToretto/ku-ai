...
### 2.5 Level 1 手动命令 — 0 confirmation 是硬规定

用户打 `/handoff` `/snapshot` `;handoff` `交接` 等命令时:

```
✗ 错误: "你确定要做 handoff 吗? 当前会话还没有命中启发式信号, 是不是太早了?"
✗ 错误: "你想要快照哪一部分的进度? 全部还是只 X?"
✓ 正确: (一句话告知已写) 已写: ~/.smart_ai/.../handoff-XXX.md
        新会话第一条粘 [prompt]. 当前会话也可以继续, 看你。
```

为什么 Level 1 严禁追问:
- 用户主动打的命令本身就是确认 — 不可能"不小心打成 /handoff"
- 追问会让用户怀疑命令是否真的生效, 下次他不敢用了
- 追问消耗的 token 比直接做还多
- "想要快照哪部分"是错误的产品思维 — handoff 永远是全量结构化快照, 不是 cherry-pick

**自由文本 reason 不算追问**: `/handoff <reason>` 里的 reason 必须原样写入 §6 `💡 用户重定向`, 这不是 ask, 是用户在主动指方向。
